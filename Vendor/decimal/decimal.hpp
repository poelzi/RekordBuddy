...
// Class definitions
// ----------------------------------------------------------------------------
template <size_t Prec> struct DecimalFactor {
...
public:
    ...
    static int64 round(T value) {
        return NxA::Internal::round(value);
    }

    static bool div_rounded(int64 &output, int64 a, int64 b) {
        return NxA::Internal::div_rounded(output, a, b);
    }
};
...
template <size_t Prec, class RoundPolicy = def_round_policy>
class decimal {
public:
    ...
    static size_t getDecimalPoints() { return Prec; }
...
// which incorrectly calculates expression like NxA::Internal::decimal<6>(a * b)
// for uint arguments. Such expression is initialized with a strange number.
//
// So instead of:
//   c = NxA::Internal::decimal<6>(a * b);
// use:
//   c = NxA::Internal::decimal_cast<6>(a * b);
template < size_t Prec >
...
template <size_t prec, typename StreamType>
void toStream(const decimal<prec> &arg, StreamType &output) {
  ...
  {
    output << dec_point;
    output << setw(static_cast<int>(arg.getDecimalPoints())) << setfill('0') << right << after;
  }
}
...
// output
template <class charT, class traits, size_t prec>
