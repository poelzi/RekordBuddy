...
{
    auto maybeAsStubPlaylist = playlist.maybeAs<MutablePlaylist*>();
    NXA_ASSERT_TRUE(maybeAsStubPlaylist.isValid());

    auto& asStubCrate = *maybeAsStubPlaylist;
    auto asStubCratePointer = static_cast<void*>(asStubCrate.get());
    auto otherParentFolder = asStubCrate->p_parentFolder;
    ...
    for (auto&& otherSubCrate : otherSubCrates) {
        if (otherSubCrate.apply([](auto&& crate) -> void* { return crate.asRawPointer(); }) == asStubCratePointer) {
            break;
        }
    ...
    asStubCrate->p_parentFolder = this;
...
{
    auto maybeAsStubFolder = folder.maybeAs<MutableFolder*>();
    NXA_ASSERT_TRUE(maybeAsStubFolder.isValid());

    auto& asStubCrate = *maybeAsStubFolder;
    auto asStubCratePointer = static_cast<void*>(asStubCrate.get());
    auto maybeOtherParentFolder = asStubCrate->p_maybeParentFolder;
    ...
    asStubCrate->p_maybeParentFolder = { this };
...
{
    auto newPlaylist = Unique<Stub::MutablePlaylist>::with(this->p_collection,
                                                           this,
                                                           Stub::MutablePlaylist::p_isProtected);
...
{
    auto newFolder = Unique<Stub::MutableFolder>::with(this->p_collection,
                                                       NotNull<MutableFolder*>{ this },
                                                       Stub::MutableFolder::p_isProtected);
