...
class MutablePlaylist : public Common::MutablePlaylist, public Common::Playlist
{
    ...
    MutableXMLNode p_rekordboxPlaylist;

    mutable Optional<MutableArray<MutableXMLNode>> p_maybeSubNodes;
    mutable boolean p_keyTypeIsTrackID;
...
protected:
    ...
    template <class T>
        inline static Optional<Common::CratePath> p_maybeCratePathForNode(const T& node, Optional<Common::CratePath> currentPath = nothing)
        {
            ...
            auto maybeParentNode = node.maybeParentNode();
...
public:
    ...
    {
        return this->p_rekordboxPlaylist.maybeStringValueForAttributeNamed("Name").valueOr("<No Name>"_String);
    }
