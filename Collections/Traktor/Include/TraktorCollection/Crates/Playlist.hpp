...
class MutablePlaylist : public Common::MutablePlaylist, public Common::Playlist
{
    ...
    friend class TraktorPlaylistTests;
#endif

    // -- Private Class Methods
    template <class T>
        inline static Optional<Common::CratePath> p_maybeCratePathForNode(const T& node, Optional<Common::CratePath> currentPath = nothing)
        {
            auto value = node.maybeStringValueForAttributeNamed("NAME");
            if (!value.isValid()) {
                return nothing;
            }

            auto path = currentPath.isValid() ? currentPath->pathWithParentNamed(*value) : Common::CratePath::forCrateNamed(*value);
            auto maybeSubNodesNode = node.maybeParentNode();
            if (maybeSubNodesNode.isValid()) {
                auto maybeParentNode = maybeSubNodesNode->maybeParentNode();
                if (maybeParentNode.isValid()) {
                    return MutablePlaylist::p_maybeCratePathForNode<XMLNode>(*maybeParentNode, path);
                }
            }
    ...
    MutableXMLNode p_traktorPlaylist;
    mutable Optional<MutableArray<MutableXMLNode>> p_maybeSubNodes;

    // -- Private Instance Methods
    MutableXMLNode p_playlistNode() const
    {
        auto traktorPlaylistNode = const_cast<MutablePlaylist*>(this)->p_traktorPlaylist;
        auto maybePlaylistNode = traktorPlaylistNode.maybeFirstSubNodeNamed("PLAYLIST");
        if (!maybePlaylistNode.isValid()) {
            maybePlaylistNode = traktorPlaylistNode.appendSubNodeNamed("PLAYLIST");
            maybePlaylistNode->setCountValueForAttributeNamed(0, "ENTRIES");
            maybePlaylistNode->setStringValueForAttributeNamed("LIST"_String, "TYPE");
        }

        return *maybePlaylistNode;
    }

    Optional<NotNull<const Common::Track*>> p_maybeTrackForNode(const MutableXMLNode&) const;
    Optional<NotNull<Common::MutableTrack*>> p_maybeTrackForNode(const MutableXMLNode&);

    MutableArray<MutableXMLNode>& p_ensureTracksAreLoaded() const;
...
public:
    ...
    MutablePlaylist(MutableXMLNode withPlaylist,
    ...
    // -- Overridden Common::Playlist Instance Methods
    ...
    {
        return this->p_traktorPlaylist.maybeStringValueForAttributeNamed("NAME").valueOr("<No Name>"_String);
    }
