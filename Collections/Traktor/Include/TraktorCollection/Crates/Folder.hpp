...
class MutableFolder : public Common::MutableFolder, public Common::Folder
{
    ...
    friend class TraktorFolderTests;
    ...
    mutable MutableXMLNode p_traktorFolder;
...
protected:
    ...
    MutableXMLNode p_subNodesNode() const
    {
        auto maybeSubNodesNode = this->p_traktorFolder.maybeFirstSubNodeNamed("SUBNODES");
        if (!maybeSubNodesNode.isValid()) {
            maybeSubNodesNode = this->p_traktorFolder.appendSubNodeNamed("SUBNODES");
            maybeSubNodesNode->setCountValueForAttributeNamed(0, "COUNT");
        }

        return *maybeSubNodesNode;
    }
...
public:
    ...
    {
        return this->p_traktorFolder.maybeStringValueForAttributeNamed("NAME").valueOr("<No Name>"_String);
    }
    void setName(const String& name) override
    {
        ...
        this->p_traktorFolder.setStringValueForAttributeNamed((*maybeParentFolder)->nextAvailableNameForFolderNamed(name), "NAME");
